    ...
    return MultiProvider(
      providers: [
        ...
        ChangeNotifierProxyProvider<AuthProvider, PortfolioProvider>(
          ...
          update: (_, auth, portfolio) =>
              portfolio!..updateToken(auth.isAuthenticated ? auth.token : null),
        ),
        ChangeNotifierProxyProvider<AuthProvider, FinanceProvider>(
          create: (_) => FinanceProvider(),
          update: (_, auth, finance) =>
              finance!..updateToken(auth.isAuthenticated ? auth.token : null),
        ),
        ChangeNotifierProxyProvider<AuthProvider, RecurringProvider>(
          create: (_) => RecurringProvider(),
          update: (_, auth, recurring) =>
              recurring!..updateToken(auth.isAuthenticated ? auth.token : null),
        ),
      ...
      child: Consumer<AuthProvider>(
        builder: (context, auth, _) {
          debugPrint(
              'Main: Building application... isLoading=${auth.isLoading}, isAuthenticated=${auth.isAuthenticated}');
          return MaterialApp(
            ...
            builder: (context, child) {
              if (auth.isLoading) {
                ...
                return Scaffold(
                  backgroundColor:
                      const Color(0xFF002F6C), // Same as login header
                  body: Center(
                    child: Column(
              ...
              debugPrint('Main: Showing Application Shell');

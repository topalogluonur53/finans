...
def update_market_from_yahoo():
    ...
    market_config = {
        ...
        'index': {
            ...
            'XTUMY.IS': 'BIST Tüm-100',
            'XGIDA.IS': 'BIST Gıda, İçecek',
            'XTKJS.IS': 'BIST Teknoloji',
            'XGMYO.IS': 'BIST GMYO',
            'XMADN.IS': 'BIST Madencilik',
            'XULAS.IS': 'BIST Ulaştırma',
            'XBLSM.IS': 'BIST Bilişim',
            'XELKT.IS': 'BIST Elektrik',
            'XFIAT.IS': 'BIST Finansal Kiralama',
            'XFINK.IS': 'BIST Mali',
            'XHARZ.IS': 'BIST Halka Arz',
            'XINSA.IS': 'BIST İnşaat',
            'XILTM.IS': 'BIST İletişim',
            'XKAGI.IS': 'BIST Orman, Kağıt, Basım',
            'XKMYA.IS': 'BIST Kimya, Petrol, Plastik',
            'XMANA.IS': 'BIST Metal Ana',
            'XMESY.IS': 'BIST Metal Eşya, Makine',
    ...
    index_constituents = {
        'XU100.IS': [
            'AEFES.IS', 'AGHOL.IS', 'AHGAZ.IS', 'AKBNK.IS', 'AKCANS.IS', 'AKFGY.IS', 'AKFYE.IS', 'AKSA.IS', 'AKSEN.IS', 'ALARK.IS', 
            'ALBRK.IS', 'ALFAS.IS', 'ARCLK.IS', 'ASELS.IS', 'ASTOR.IS', 'BERA.IS', 'BIMAS.IS', 'BRMEN.IS', 'BRSAN.IS', 'BUCIM.IS', 
            'CANTE.IS', 'CCOLA.IS', 'CEMTS.IS', 'CIMSA.IS', 'CWENE.IS', 'DOAS.IS', 'DOHOL.IS', 'ECILC.IS', 'ECZYT.IS', 'EGEEN.IS', 
            'EKGYO.IS', 'ENJSA.IS', 'ENKAI.IS', 'EREGL.IS', 'EUPWR.IS', 'EUREN.IS', 'FROTO.IS', 'GARAN.IS', 'GENIL.IS', 'GESAN.IS', 
            'GLYHO.IS', 'GUBRF.IS', 'GWIND.IS', 'HALKB.IS', 'HEKTS.IS', 'HKTM.IS', 'HLGYO.IS', 'IPEKE.IS', 'ISCTR.IS', 'ISDMR.IS', 
            'ISFIN.IS', 'ISGYO.IS', 'ISMEN.IS', 'IZENR.IS', 'KALES.IS', 'KARSN.IS', 'KAYSE.IS', 'KCAER.IS', 'KCHOL.IS', 'KMPUR.IS', 
            'KONYA.IS', 'KORDS.IS', 'KOZAA.IS', 'KOZAL.IS', 'KRDMD.IS', 'KZBGY.IS', 'MAVI.IS', 'MGROS.IS', 'MIATK.IS', 'ODAS.IS', 
            'OTKAR.IS', 'OYAKC.IS', 'PENTA.IS', 'PETKM.IS', 'PGSUS.IS', 'PNLSN.IS', 'QUAGR.IS', 'SAHOL.IS', 'SASA.IS', 'SELEC.IS', 
            'SISE.IS', 'SKBNK.IS', 'SMRTG.IS', 'SOKM.IS', 'TAVHL.IS', 'TCELL.IS', 'THYAO.IS', 'TKFEN.IS', 'TOASO.IS', 'TRGYO.IS', 
